...
export default function BlogDetailPage() {
  ...
  // Fire a single blog_view analytics event once the post is available.
  const viewed = useRef(false);
  useEffect(() => {
    if (post && !viewed.current) {
      viewed.current = true;
      analytics.blogView(post._id || id, post.title);
    }
  }, [post, id]);
  ...
  return (
    <>
      ...
      <section className={`relative pt-32 pb-20 overflow-hidden ${post.image ? "" : "gradient-mesh"}`}>
        {post.image && (
          <>
            <Image src={cloudinaryUrl(post.image, { width: 1600 })} alt={post.title} fill priority sizes="100vw" className="object-cover" />
      ...
      <section className="py-16 bg-white">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          <AnimatedSection>
            {post.contentHtml ? (
              // contentHtml is sanitized server-side on save (see lib/sanitizeHtml.js).
