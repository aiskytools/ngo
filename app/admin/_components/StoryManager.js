  ...
  return (
    <div>
      ...
      <form key={editing?._id || "new"} onSubmit={submit} className="bg-white rounded-3xl p-8 border border-gray-100 shadow-sm mb-8">
        ...
        <textarea name="background" required rows={3} maxLength={5000} placeholder="Background — their situation before *" defaultValue={editing?.background || ""} className={`${field} resize-none`} />
        <textarea name="intervention" required rows={3} maxLength={5000} placeholder="What we did *" defaultValue={editing?.intervention || ""} className={`${field} resize-none`} />
        <textarea name="outcome" required rows={2} maxLength={2000} placeholder="Outcome today *" defaultValue={editing?.outcome || ""} className={`${field} resize-none`} />
      ...
      <h3 className="font-bold text-lg mb-4">Stories ({stories.length})</h3>
      {stories.length === 0 ? (
